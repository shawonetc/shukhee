          ...
          {QUICK_CONSULTATIONS.map((c) => {
            ...
            return (
              <Link
                key={c.id}
                href={linkPath}
                className="flex-shrink-0 block no-underline cursor-pointer"
              >
                <div
                  className={`w-[330px] h-[180px] p-6 rounded-2xl ${c.bgColor} relative overflow-hidden group/card flex flex-col justify-between hover:shadow-sm transition-all duration-355`}
                >
                  {/* Left Side Info Area */}
                  ...
                  {/* Bottom Left Button (rendered as div inside link) */}
                  <div className="z-10">
                    <div className="inline-flex bg-white border border-cyan-400 text-cyan-500 hover:bg-cyan-500 hover:text-white font-bold text-xs py-1.5 px-4 rounded-lg shadow-2xs transition-all items-center gap-1">
                      <span>{c.cta}</span>
